...
- 現在の試験モデル: Gemma 4 E2B Instruct
- バックエンド接続先: `http://127.0.0.1:8081/v1/chat/completions`
- バックエンドモデル名: `gemma-4-e2b-it`
- チャット応答の標準経路: Frontend -> Go BFF -> llama.cpp -> Gemma

LLM が起動していない場合、チャットはデモ応答に落とさず接続エラーとして扱います。
...
- `llm`: `llama.cpp` サーバー。Gemma 4 E2B の軽量 GGUF を Hugging Face から取得して起動
...
- 初回起動時は Gemma モデルのダウンロードに時間がかかります
- 既定では `unsloth/gemma-4-E2B-it-GGUF` の `Q4_K_M` を使います
- 初回起動時はダウンロードに時間がかかるので、`make dev-up` は LLM の待ち時間を長めに取っています
